...
class ApiController extends AbstractController
{
    ...
    public function sendMessage(
        ...
        if ('dev' === $this->kernel->getEnvironment()) {
            $this->logger->info('📨 Получен запрос на /chat/message', [
        ...
        if ('OPTIONS' === $request->getMethod()) {
            $this->logger->debug('🔄 Обработан OPTIONS запрос');
        ...
        try {
            ...
            if (empty($message)) {
                $this->logger->warning('❌ Пустое сообщение', ['input' => $input]);
            ...
            if ('dev' === $this->kernel->getEnvironment()) {
                $this->logger->info('🔍 Проверка темы сообщения', ['message' => $message]);
                if ($this->topicService->isForbidden($message)) {
                    $this->logger->warning('🚫 Запрещенная тема', ['message' => $message]);
                ...
                $this->logger->info('⚙️ Подготовка ответа', ['message' => $message]);

                $this->logger->info('✅ Успешный ответ', [
    ...
    // Create session with client and operator
